...
mh_algoZetaMat <- function(dataMat, zeta, theta, ds){
  ...
  for(t in 1:mhloop){
    zetaCand = rbeta(length(zetaCurr),alpha_base, beta_base)
...
mh_cluster2 = function(datanum1, datanum2, theta, alpha_0){
  ...
  store_theta_prob = matrix(NA, nrow = num_obs, ncol = cl_num)
  ...
  for (j in 1:num_obs){
    ...
    #Sample new cluster 
    new_c = sample(1:cl_num, 1, FALSE, exp(prob_norm))
    ...
    store_theta_prob[j,] = exp(prob_norm)
  }
    ...
    return(list(dataMat1 = dataMat1, dataMat2 = dataMat2,
                store_theta_prob = store_theta_prob))
}
